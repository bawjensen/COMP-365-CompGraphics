...
def writeToFile(filename, vertices, normals, faces, normalIndices, materialList, usingMTL):
	...
	with open(filename, 'w') as outFile:
		...
		# if textureIndices:
		# 	# Texture Indices
		# 	outFile.write('\ttexture_indices {\n')
		# 	outFile.write('\t\t' + str(len(textureIndices)) + ',\n')

		# 	for textureIndex in textureIndices:
		# 		outFile.write('\t\t<' + ', '.join(textureIndex) + '>,\n')
...
def main():
	...
	materialList = []
	if usingMTL:
		materialList, matNameDict = parseMTL('table.mtl')
	...
	if faces[0][0].find('//') != -1:
		if usingMTL:
			normalIndices = [[int(index.split('//')[1])-1 for index in face[:-1]] for face in faces]
	...
	elif faces[0][0].find('/') != -1:
		if usingMTL:
			normalIndices = [[int(index.split('/')[2])-1 for index in face[:-1]] for face in faces]
			faces = [[int(index.split('/')[0])-1 for index in face[:-1]] + [face[-1]] for face in faces]
		else:
			normalIndices = [[int(index.split('/')[2])-1 for index in face] for face in faces]
			faces = [[int(index.split('/')[0])-1 for index in face] for face in faces]
